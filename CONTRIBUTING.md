...
Thanks for the interest — this lab gets better when more people add to it.
...
- **Better hunt packs.** Real-world SPL queries you've used in investigations are gold here. Same for VQL artifacts and PowerShell live-response one-liners.
...
- **Bug fixes and clarifications** in `BUILD.md`. If you build the lab and hit something the guide doesn't cover, that's worth a PR.
...
- **Keep `BUILD.md` linear.** It's the most-read document. New phases or steps belong there only if they're essential to first-time setup.
...
- **Commit `.env.example` updates** whenever you add new env vars. Don't commit `.env`.
...
- Could be repurposed offensively against systems the user doesn't own
