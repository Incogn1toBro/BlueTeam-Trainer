...
class AtomicRunner:
    ...
    def _clean_stderr(raw: str) -> str:
        ...
        WinRM-invoked PowerShell wraps verbose/progress/info records as a CLIXML
        document on the error stream. This is metadata, not actual errors -
        it confuses analysts who see big red 'STDERR' panels for healthy runs.

        We extract the human-readable message blocks if there are any, otherwise
        return empty. Genuine errors from PowerShell still come through this
        path, so we keep anything that looks like a real exception message.
        """
        if not raw:
            return ""

        # If it's pure CLIXML framing with no useful content, drop it entirely.
        if raw.lstrip().startswith("#< CLIXML"):
            import re
            # Pull out anything inside <S S="Error">...</S> tags - these are
            # the genuine error records embedded in the CLIXML.
    ...
    def test_connection(self) -> bool:
        """Return True if we can run a trivial PS command on the victim."""
        result = self._run_ps("$env:COMPUTERNAME")
        if result.success:
            log.info("Victim hostname: %s", result.stdout.strip())
        return result.success

    # Patterns that indicate genuine test failure, even when PowerShell
    # exit code is 0. Atomic Red Team often swallows these into stdout.
    # Each pattern maps to a short human-readable failure category.
    _FAILURE_PATTERNS = [
        # Network / DNS - by far the most common on isolated lab networks
        (r"(?i)remote name could not be resolved", "DNS resolution failed - victim has no internet access"),
        (r"(?i)unable to connect to the remote server", "Connection refused / network unreachable"),
        (r"(?i)the operation has timed out", "Network timeout"),
        (r"(?i)could not establish trust relationship for the SSL", "TLS handshake failed (cert issue or no internet)"),
    ...
    def execute_atomic(
        ...
        Success/failure detection works in three layers:
          1. PowerShell exit code (least reliable - Atomic often returns 0 even on failure)
          2. Sentinel strings in stdout (ATOMIC_TEST_COMPLETE / ATOMIC_PRECHECK_FAILED)
          3. Pattern matching against known failure indicators in output (DNS,
             HTTP errors, Defender blocks, etc.) - this is what catches the
             "test reported success but actually failed" cases.
        """
        import re
...
""".strip()
        ...
        # Combined output for pattern matching - some failure messages appear
        # in stderr (PowerShell error stream) rather than stdout.
        combined = result.stdout + "\n" + result.stderr

        # Layer 3: Look for known failure patterns - this catches cases where
        # the test "completed" but actually failed (most common: DNS/network failures
        # on an isolated victim). Run this BEFORE checking the success sentinel.
        failure_reason = None
        for pattern, reason in self._FAILURE_PATTERNS:
            if re.search(pattern, combined):
                failure_reason = reason
                break
        ...
        if failure_reason:
            result.success = False
            # Surface the reason in stderr so the UI shows it clearly
            if not result.stderr.strip():
                result.stderr = f"[Detected failure] {failure_reason}"
            else:
                result.stderr = f"[Detected failure] {failure_reason}\n\n{result.stderr}"
        elif "ATOMIC_TEST_COMPLETE" in result.stdout:
            result.success = True
            ...
            result.success = False
        # else: trust whatever _run_ps already set based on exit code
...
""".strip()
        result = self._run_ps(script)
        if "PREREQ_CHECK_COMPLETE" in result.stdout:
            result.success = True
        return result
